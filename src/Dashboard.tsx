...
export default function Dashboard({ onLogout }: { onLogout?: () => void }) {
  ...
  const appointments = globalAppointmentsList
    ...
    .map(a => {
      ...
      const serviceObj = servicesList.find(s => s.name === a.service) || { name: a.service, price: '₡ 6,000', duration: '30 min' };
                  ...
                  <div className="relative w-full h-52 rounded-3xl overflow-hidden border border-white/10 shadow-2xl mb-8">
                    ...
                    <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/30 to-transparent flex items-end p-5">
                      <div className="transform transition-transform duration-500 translate-y-0">
                        <span className="bg-white text-black text-[10px] font-black px-2 py-1 rounded-md uppercase tracking-wider backdrop-blur-md">Cartelera</span>
                        <p className="text-white font-medium mt-2 text-sm drop-shadow-md">Estilo y Tendencia</p>
                      </div>
                    </div>
                    {/* Dots */}
              ...
              {step > 0 && (
                <div className="bg-white/[0.03] backdrop-blur-2xl border border-white/10 rounded-[2rem] p-5 shadow-2xl relative overflow-hidden animate-in fade-in slide-in-from-bottom-8 duration-500">
                  ...
                  <div className="h-6 flex items-center mb-6">
                    {step > 1 && step < 4 && (
                      <button onClick={() => setStep(prev => prev - 1)} className="absolute top-5 left-5 text-slate-400 hover:text-white transition-colors flex items-center gap-1 text-xs font-semibold z-20 hover:-translate-x-1 duration-300">
                                ...
                                <div className="grid grid-cols-3 gap-2">
                                  {TIMES.map(t => {
                                    ...
                                    const slotDuration = booking.service 
                                      ? getServiceDurationMinutes(booking.service.name, servicesList) 
                                      : 30;
