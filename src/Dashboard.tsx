...
export default function Dashboard({ onLogout, onReady }: { onLogout?: () => void; onReady?: () => void }) {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      if (user) {
        ...
        const [
          ...
          // Fetch all appointments globally to detect busy slots across clients
          supabase.from('appointments').select('id, barber_id, barber_name, client_name, client_id, service_name, time, date, status').order('created_at', { ascending: false }),
    ...
    // Subscribe to Realtime — across all key app tables
    const channel = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'promotions' }, () => loadData())
      .on('postgres_changes', { event: '*', schema: 'public', table: 'blocked_hours' }, () => loadData())
      .on('postgres_changes', { event: '*', schema: 'public', table: 'permisos' }, () => loadData())
  ...
  const handleTimeSelectSave = async (time: string) => {
    ...
    if (newBooking.barber && newBooking.date && newBooking.service && userId) {
      const targetDateStr = toLocalDateStr(newBooking.date);

      // Verify slot is still available in DB before inserting
      const { data: existingAppts } = await supabase
        .from('appointments')
        .select('id')
        .eq('barber_id', newBooking.barber.id)
        .eq('date', targetDateStr)
        .eq('time', time)
        .neq('status', 'Cancelada');

      if (existingAppts && existingAppts.length > 0) {
        alert("Lo sentimos, este espacio acaba de ser reservado por otro cliente. Por favor selecciona otra hora.");
        setStep(3);
        return;
      }
                            ...
                            {(() => {
                              ...
                              const availableTimes = TIMES.filter(t => {
                                ...
                                const isBusy = globalAppts.some((a: any) => {
                                  if (String(a.barberId) !== String(barberId) || a.date !== dateKey || a.status === 'Cancelada') return false;
                                ...
                                return !isPast && !isOutsideHours && !isLunchOverlap && !isBusy && !isBlocked;
                              });
                              ...
                              if (availableTimes.length === 0) {
                                return (
                                  <div className="py-8 px-4 border border-dashed border-white/10 rounded-[2rem] text-center bg-white/[0.01]">
                                    <CalendarOff className="w-8 h-8 text-slate-500 mx-auto mb-2" />
                                    <p className="text-xs text-slate-400 font-bold">No hay horas disponibles para esta fecha.</p>
                                  </div>
                                );
                              }
