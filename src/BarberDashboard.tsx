...
export default function BarberDashboard({ onLogout, user, isAdmin = false }: { onLogout: () => void, user: any, isAdmin?: boolean }) {
          ...
          {activeNav === 'rendimiento' && (
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 pb-10 px-2">
              ...
              {(() => {
                ...
                const monthReceipts = [
                  { id: 'REC-001', date: '2026-05-15', formattedDate: '15 de Mayo, 2026', amount: 145000, status: 'Pagado' },
                  { id: 'REC-002', date: '2026-04-30', formattedDate: '30 de Abril, 2026', amount: 295900, status: 'Pagado' },
                  { id: 'REC-003', date: '2026-04-15', formattedDate: '15 de Abril, 2026', amount: 120000, status: 'Pagado' }
                ].filter(r => {
                  const prefix = `${year}-${String(month+1).padStart(2,'0')}`;
                ...
                const commissionServices = monthAppointments.filter(a => a.status === 'completed').reduce((sum, a) => sum + (a.commission || 0), 0);
                const commissionProducts = monthSales.reduce((sum, s) => sum + (s.commission || 0), 0); // Dynamic commission sum!
                    ...
                    <div className="space-y-3">
                      {monthReceipts.length === 0 ? (
                        ...
                        monthReceipts.map(rec => (
                          <button key={rec.id} className="w-full bg-black/40 border border-white/5 hover:border-white/20 transition-all rounded-2xl p-4 text-left flex justify-between items-center group relative overflow-hidden">
                               ...
                               <button
                                 onClick={() => {
                                   ...
                                   if (confirm(`¿Confirmas realizar este pago por un total de ₡${totalPeriodPay.toLocaleString()} a ${collab.name}?`)) {
                                     ...
                                     setCollaboratorPayments([newPayment, ...collaboratorPayments]);
                                     setPayStartDateInput('');
                                     setPayEndDateInput('');
                                     setPayBasePayInput('');
